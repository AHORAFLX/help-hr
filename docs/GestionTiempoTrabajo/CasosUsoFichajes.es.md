...
- **Empleados Planificados**: Estos tienen asignado un turno (horario) para una fecha determinada. Cuando el empleado ficha se le asigna el turno planificado y la fecha de jornada del fichaje. Si ficha fuera de los límites del turno, el sistema permitirá fichar, pero se mostrará una incidencia en la jornada del empleado.
    ...
    - Existe una separación con el fichaje anterior que supera la separación mínima entre jornadas (`MinBreakBetweenWorkingDays`).
    - La duración total desde el primer fichaje de la jornada excede las `MaxJourneyHours` y el fichaje actual es de un día diferente al de inicio de la jornada.
...
- **Empleados sin Planificar**: Estos al fichar por primera vez se les asigna un turno genérico "Sin Planificación" y se les asigna la fecha de jornada del fichaje.
    ...
    - Se realiza un fichaje con una separación respecto al fichaje anterior que supera la separación mínima entre jornadas (`MinBreakBetweenWorkingDays`).
    - La duración total desde el primer fichaje de la jornada excede las `MaxJourneyHours` y el fichaje actual es de un día diferente al de inicio de la jornada.
...
Fichar antes del límite inferior del turno asignará el turno planificado y mostrará una incidencia de Fichajes fuera de límites del turno. Este caso también contempla el caso de que la salida sea fuera del límite superior del turno.
...
- Existe una separación con el fichaje anterior que excede la `MinBreakBetweenWorkingDays` (en el ejemplo, 8 horas).
- La duración total desde el primer fichaje de la jornada excede las `MaxJourneyHours` (en el ejemplo, 12 horas) y el fichaje actual es en una fecha diferente a la del inicio de la jornada.
...
2. Lanzando el proceso RJ sobre la jornada actual: En este caso En y Sn _pasaran a formar parte de la jornada actual_ junto a E1, S1, E2 y S2 porque pertenecen a la misma VJ.
